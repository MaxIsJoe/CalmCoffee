<script lang="ts">
	...
	export let maxLength: number = 3680;
	...
	$: if (draftKey && (value !== prevValue || styles !== prevStyles)) {
		...
		draftSaveTimeout = setTimeout(() => {
			...
			localStorage.setItem(
				draftKey!,
				JSON.stringify({ value, styles, lastSaved: lastDraftSave.toISOString() })
			);
			isSavingDraft = false;
			prevValue = value;
			prevStyles = styles;
...
<div class="block-editor">
	<ZenMarkdownEditor
		...
		placeholder={isEditing
			? 'Edit your block content...'
			: ''.concat('Write your next ', maxLength.toString(), ' characters (markdown supported)')}
		customStyles={(() => {
			try {
